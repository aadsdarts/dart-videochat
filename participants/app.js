...
function setupRealtimeChannel() {
    ...
    // Also support spectator-specific answers
        // Spectator offer request -> create/send dedicated offer for spectator
        state.channel.on('broadcast', { event: 'spectator-offer-request' }, async (payload) => {
            const token = payload.payload?.token;
            if (!token) return;
            await ensureSpectatorPeer(token);
            await sendSpectatorOffer(token);
        });
        ...
        state.channel.on('broadcast', { event: 'spectator-answer' }, async (payload) => {
            ...
            const peer = state.spectatorPeers[token];
            if (!peer || !peer.pc) return;

            await peer.pc.setRemoteDescription(new RTCSessionDescription(answer));
            peer.remoteDescriptionSet = true;
            await flushSpectatorIce(token);
        });

        // Spectator ICE from remote
    ...
    state.channel.on('presence', { event: 'sync' }, () => {
        ...
        // Respond to lobby ping to confirm active participants
        state.channel.on('broadcast', { event: 'lobby-ping' }, () => {
            try {
                state.channel.send({
                    type: 'broadcast',
                    event: 'lobby-pong',
                    payload: { role: 'participant', ts: Date.now() }
                });
            } catch (e) {
                console.warn('Failed to send lobby-pong', e);
            }
        });
    });
...
// Spectator helpers
async function ensureSpectatorPeer(token) {
    if (!state.localStream) {
        console.warn('No local stream for spectator peer');
        return null;
    }

    if (state.spectatorPeers[token]?.pc) {
        return state.spectatorPeers[token];
    }
    ...
    pc.onicecandidate = (event) => {
        if (event.candidate) {
            state.channel?.send({
                type: 'broadcast',
                event: 'spectator-ice',
                payload: { token, candidate: event.candidate }
            });
        }
    };

    pc.onconnectionstatechange = () => {
        console.log(`Spectator ${token} connection:`, pc.connectionState);
    };

    pc.oniceconnectionstatechange = () => {
        console.log(`Spectator ${token} ICE:`, pc.iceConnectionState);
    };

    state.spectatorPeers[token] = peer;
    return peer;
}

async function sendSpectatorOffer(token) {
    const peer = await ensureSpectatorPeer(token);
    if (!peer || !peer.pc) return;
    ...
    state.channel?.send({
        type: 'broadcast',
        event: 'spectator-offer',
        payload: { token, offer: peer.pc.localDescription }
    });
}

async function flushSpectatorIce(token) {
    const peer = state.spectatorPeers[token];
...
async function handleEndCall() {
    ...
    // Close spectator peers
    Object.values(state.spectatorPeers || {}).forEach(peer => {
        try {
            peer.pc?.close();
        } catch (err) {
            console.warn('Error closing spectator peer', err);
        }
    });
    state.spectatorPeers = {};
